  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100 dark:bg-slate-900">
      <div className="w-full max-w-md bg-white dark:bg-slate-800 p-8 rounded shadow">
        <h2 className="text-2xl mb-4 text-slate-900 dark:text-white">Create account</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <input name="username" placeholder="Username" required onChange={(e)=>setForm({...form,username:e.target.value})} className="p-3 rounded bg-slate-100 dark:bg-slate-700"/>
          ...
          <button className="mt-2 p-3 bg-indigo-600 text-white rounded">Register</button>
        </form>
      </div>
    </div>
  )
